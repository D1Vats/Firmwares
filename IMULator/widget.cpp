    ...
    ,m_Timer(new QTimer(this))
{
    ...
    connect(ui->Simulator_Button, &QPushButton::clicked, this, &Widget::simulatorButton_Clicked);
...
void Widget::closeSerialPort()
{
    ...
    {
        m_Serial->close();
        plotter->newDataAvailable = false;
...
void Widget::plotterButton_Clicked()
{
    plotter->show();
    plotter->raise();
...
void Widget::simulatorButton_Clicked()
{
    simulator->showMaximized();
    simulator->raise();
...
void Widget::readData()
{
    while(m_Serial->canReadLine())
    {
        ...
        //qDebug() << "The data coming from serial port: " << data;
        monitor->getData(data);
        ...
        if(value.size() == 3)
        {
            ...
            {
                qDebug() << "Error parsing data. Values:" << value;
            }
        }
    }
}
